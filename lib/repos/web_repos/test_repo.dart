...
import 'package:stripes_main_config/repos/web_repos/query_utils.dart';
...
class Test extends TestRepo<repo.BlueDyeTest> {
  ...
  setValue(repo.BlueDyeTest obj) async {
    ...
    try {
      final GraphQLResponse<BlueDyeTest> updated =
          await Amplify.API.mutate(request: update).response;
      if (updated.data == null) {
        safePrint(updated.errors);
        return;
      }
      ...
      for (BlueDyeTestLog log in logs) {
        final BlueDyeTestLog toCreate = log.copyWith(blueDyeTest: updated.data);
      ...
      final BlueDyeTest? toEmit = updated.data?.copyWith(logs: created);
      _emit(toEmit);
    } catch (e) {
      safePrint(e);
    }
